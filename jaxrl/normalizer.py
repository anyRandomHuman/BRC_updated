...
class RewardNormalizer(object):
    def __init__(self, num_seeds: int, target_entropy: float, discount: float = 0.99, v_max: float = 10.0,
        ...
        self.step = 0
        self.rewards = np.zeros((num_seeds, max_steps), dtype=np.float32) if max_steps is not None else [[] for _ in
                                                                                                         range(
                                                                                                             num_seeds)]
    ...
    def normalize(self, batches: Batch, temperature: np.ndarray):
        denominator = np.where(self.returns_max_norm > np.abs(self.returns_min_norm), self.returns_max_norm,
        ...
        return Batch(observations=batches.observations, actions=batches.actions, rewards=rewards, masks=batches.masks,
